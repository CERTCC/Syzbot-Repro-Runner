...
DIR=chroot
PREINSTALL_PKGS=openssh-server,curl,tar,gcc,g++,libc6-dev,time,strace,sudo,less,psmisc,selinux-utils,policycoreutils,checkpolicy,selinux-policy-default,make,git,libpthread-stubs0-dev

# If ADD_PACKAGE is not defined as an external environment variable, use our default packages
if [ -z ${ADD_PACKAGE+x} ]; then
    ADD_PACKAGE="make,git,vim,tmux,tcpdump"
fi
